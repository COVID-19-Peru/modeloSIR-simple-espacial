...
program nlsir, rclass
	...
	replace `inf' = (1-`g')*`inf_past' + `beta'*`coef_b'*`sanos' `if'
end
...
** ESTIMACION
nl sir @ InfAcum InfAcum_past SanosAcum_past if first!=1, parameters(pi beta)
...
predictnl infhat = (1-`g')*InfAcum_past + `beta'*`coef_b'*SanosAcum_past if first!=1, se(infhat_se)
drop D*
save infectados_MINSA_30Apr_predicNL, replace

* OUT OF SAMPLE
*clear all
use infectados_MINSA_30Apr_predicNL, clear
keep if t==55
drop t
