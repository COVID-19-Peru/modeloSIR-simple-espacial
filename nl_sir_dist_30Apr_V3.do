...
program nlsir, rclass
	...
	tempname pi beta g delta eta alpha
	scalar `pi' = `at'[1,1]
	scalar `beta' = `at'[1,2]
	scalar `g' = 0.04
	scalar `delta' = `at'[1,3]
	...
	replace `inf' = (1-`g')*`inf_past' + (`beta'+`eta'*t+`alpha'*t^2)*`coef_b'*`sanos'*(`densidad'^`delta') `if'
end
...
** ESTIMACION
nl sir @ InfAcum InfAcum_past SanosAcum_past ldensidad if first!=1, parameters(pi beta delta eta alpha)
...
predictnl infhat = (1-`g')*InfAcum_past + (`beta'+`eta'*t+`alpha'*t^2)*`coef_b'*SanosAcum_past*(ldensidad^`delta') if first!=1, se(infhat_se)
drop D* // i0* i1* i2*
save infectados_MINSA_30Apr_predicNL_V3, replace

* OUT OF SAMPLE
*clear all
use infectados_MINSA_30Apr_predicNL_V3, clear
keep if t==55
drop t
...
local t_1 = `t' - 1
	...
	g Infectados_t`t' = (1-`g')*Infectados_t`t_1' + (`beta'+`eta'*`T'+`alpha'*`T'^2)*`coef_b'*Sanos_t`t_1'*(ldensidad^`delta')
	g Recuperados_t`t' = Recuperados_t`t_1' + `g'*Infectados_t`t_1'
	g Sanos_t`t' = Poblacion - Infectados_t`t' - Recuperados_t`t'
}
}
cap drop _*
save predicNLInfectados_30Apr30may_V3, replace
